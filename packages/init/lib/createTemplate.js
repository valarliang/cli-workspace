...
export default async function createTemplate(name, opts) {
  ...
  if (!type) type = await getAddType(); // 创建类型（选项指引）
  if (!ADD_TYPE.some((t) => t.value === type))
    throw Error(`创建类型 ${type} 不存在！`);
  log.verbose("addType", type);
  if (type === "project") {
    // 命令参数未定义名称则需输入项目名称
    if (!name) {
      name = await getAddName();
      log.verbose("name", name);
    }
    if (!template) template = await getAddTemplate(ADD_TEMPLATE); // 选择的模板（选项指引）
